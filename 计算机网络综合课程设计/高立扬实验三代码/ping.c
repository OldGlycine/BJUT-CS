...
int main(int argc, char **argv)
{
	...
	while ( (c = getopt(argc, argv, "vhbc:i:qs:t:l:")) != -1) {
		switch (c) {
...
void print_help(){
    printf("usage: ping [opt] <hostname>\n");
    printf("opt:\n");
    printf("\t-v\tNormal mode\n");
    printf("\t-b\tBroadcast\n");
    printf("\t-c <number>\tSend the specified number of packets\n");
    printf("\t-i <interval>\tSet the sending interval, in seconds\n");
    printf("\t-q\tQuiet mode\n");
    printf("\t-s <size>\tSet size of ICMP\n");
...
void sigint_handler(int signum) {
    if(1){
    	...
    	printf("%d packets transmitted, %d received, %.1f%% loss, rtt %.3lfms\n",
           already_sent, packets_received, ((already_sent - packets_received) / already_sent) * 100.0, total_time);
    }
    exit(signum);
}
...
send_v4(void)
{
	...
	if(count&&already_sent>count){
		printf("--- %s  ping statistics---\n", name);
    	printf("%d packets transmitted, %d received, %.1f%% loss, rtt %.3lfms\n",
           already_sent-1, packets_received, ((already_sent - packets_received) / already_sent) * 100.0, total_time);
		exit(0);
	}
}
